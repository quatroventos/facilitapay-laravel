...
cd /usr/share/nginx/html/argon_dashboard_2_pro_laravel/ # go to project repository on the server
git fetch --all
php8.0 artisan down
...
chown -R www-data:www-data /usr/share/nginx/html/argon_dashboard_2_pro_laravel/ #change here too project repository
php8.0 artisan up
source ~/.nvm/nvm.sh
npm install
npm run prod
