...
@section('content')
    ...
    <nav class="navbar navbar-main navbar-expand-lg  px-0 mx-4 shadow-none border-radius-xl z-index-sticky " id="navbarBlur"
        ...
        <div class="container-fluid py-1 px-3">
            @include('backend.layouts.navbars.auth.topnav', ['title' => 'New Category'])
        ...
        <div class="row mb-5">
            <div class="col-lg-9 col-12 mx-auto">
                <div class="card card-body mt-4">
                    ...
                    <form method="POST" action="{{ route('block-edit.update', $block->id) }}" enctype="multipart/form-data">
                        @csrf

                        <img src="{{ $block->pictureUrl() }}" class="avatar-xxl border-radius-lg shadow-sm mt-4">
                        <div class="d-flex flex-column mb-3">
                            <label class="mt-4 form-label" for="picture">Imagem</label>
                            <input type="file" name="image" accept="image/*" id="picture" class="form-control">
                        </div>

                        <label for="title" class="form-label">Nome</label>
                        <div class="mb-3">
                            <input type="text" class="form-control" id="title" name="title" value="{{ old('title',$block->title)}}">
                            @error('title')
                        ...
                        <div class="mb-3">
                            <input type="text" class="form-control" id="category" name="category" value="{{ old('category',$block->category)}}">
                            @error('category')
                            <p class='text-danger text-xs pt-1'> {{ $message }} </p>
                            @enderror
                        </div>
                        ...
                        <div style="position: relative; border: 1px solid #ccc; min-height: 400px; border-radius: 10px; overflow: hidden;">
                            <div id="editor">{{ old('content', $block->content) }}</div>
                        </div>
                        @error('content')
                        <p class='text-danger text-xs pt-1'> {{ $message }} </p>
                        @enderror

                        <input type="hidden" name="content" id="content" value="{{ old('content', $block->content) }}">
